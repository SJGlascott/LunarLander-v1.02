...
Public Class LanderClient
    ...
    Dim renderLander As LanderImageProcessor = New LanderImageProcessor
    Dim renderOpponent As LanderImageProcessor = New LanderImageProcessor
    ...
    Dim sendCrashed As Integer
    Dim sendThursting As Integer
    Dim lander2Angle As Double
    Dim lander2Crashed As Boolean
    Dim lander2Thrusting As Boolean

    Dim pointsRec As Boolean = False
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        renderLander.preloadLanderImages()
        renderOpponent.preloadOpponentImages()

        'processes resource files and takes width as parameter
        renderLander.loadLanderImages(40)
        renderOpponent.loadLanderImages(40)
    ...
    'This sub class handels reciving the array of random points from the server, once recieved the game builds the level and begind
    'the game
    Private Sub recPoints()
        ...
        If recieved IsNot "" Then
            Dim recArray() As String = Split(recieved, "SPLIT")
            For i As Integer = 0 To 128
                Dim splitXY() As String = Split(recArray(i), ",")
                Dim x As String = splitXY(0).TrimStart({"{"c, "X"c, "="c})
                Dim y As String = splitXY(1).TrimStart({"Y"c, "="c}).TrimEnd({"}"c})
                Dim combine As String = x & "," & y
                pathPoints(i) = CType(converter.ConvertFromString(combine), Point)
            Next
        End If
    ...
    Private Sub TimerRec_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles timerRec.Tick
        ...
        If crashed Then
            ...
            sendBytes = System.Text.Encoding.ASCII.GetBytes(landerLocation.X & " " & landerLocation.Y & " " & angle & " " & sendCrashed & " " & sendThursting)
            ...
            If recieved IsNot "" Then
                ...
                Dim X, Y, isCrashed, isThrusting As String
                Dim l2angle As Double
                X = recArray(0)
                Y = recArray(1)
                l2angle = CDbl(recArray(2))
                isCrashed = recArray(3)
                isThrusting = recArray(4)
                landerLocation2.X = CInt(X)
                landerLocation2.Y = CInt(Y)
                lander2Angle = l2angle

                If isCrashed = 1 Then
                    lander2Crashed = True
                Else
                    lander2Crashed = False
                End If

                If isThrusting = 1 Then
                    lander2Thrusting = True
                Else
                    lander2Thrusting = False
                End If
    ...
    Private Sub GameLoop()
        ...
        Do While (Me.Created)
            ...
            GameLogic()
            RenderScene()
            picLander.Image = renderLander.getNewLanderImage(angle, crashed, thrusting)
            landerPic2.Image = renderOpponent.getNewLanderImage(lander2Angle, lander2Crashed, lander2Thrusting)
